...
public class VCalendarPanel extends FocusableFlexTable implements
    ...
    private ClickHandler dayClickHandler = new ClickHandler() {
        ...
        public void onClick(ClickEvent event) {
            ...
            if (!isDateInsideRange(newDate, DateTimeResolution.DAY)) {
                return;
            }
    ...
    private void focusDay(Date date) {
        ...
        if (resolution.getCalendarField() > DateTimeResolution.MONTH
                .getCalendarField()) {
            if (focusedDay != null) {
    ...
    public void setShowISOWeekNumbers(boolean showISOWeekNumbers) {
        ...
        if (initialRenderDone && getResolution()
                .getCalendarField() >= DateTimeResolution.DAY.getCalendarField()) {
    ...
    private boolean isAcceptedByRangeStart(Date date,
            DateTimeResolution minResolution) {
        assert (date != null);
    ...
    private void buildCalendarBody() {
        ...
        for (int i = 0; i < 7; i++) {
            ...
            if (getResolution().getCalendarField() > DateTimeResolution.MONTH
                    .getCalendarField()) {
    ...
    private boolean isTimeSelectorNeeded() {
        return getResolution().getCalendarField() > DateTimeResolution.DAY
                .getCalendarField();
    }
    ...
    public void renderCalendar(boolean updateDate) {
        ...
        if (updateDate && getResolution().getCalendarField() <= DateTimeResolution.MONTH
        ...
        final boolean needsMonth = getResolution()
                .getCalendarField() > DateTimeResolution.YEAR.getCalendarField();
        boolean needsBody = getResolution().getCalendarField() >= DateTimeResolution.DAY
                .getCalendarField();
    ...
    protected boolean handleNavigation(int keycode, boolean ctrl,
        ...
        else if (resolution == DateTimeResolution.YEAR) {
    ...
    public void setDate(Date currentDate) {
        ...
        if (value == null || currentDateWasAdjusted) {
            ...
            if (rangeStart != null || rangeEnd != null) {
                ...
                // a day
                // resolution
                if (getResolution().getCalendarField() >= DateTimeResolution.DAY
                        .getCalendarField()) {
                    value = null;
                }
            } else {
    ...
    public class VTime extends FlowPanel implements ChangeHandler {
        ...
        private void buildTime() {
            ...
            if (getResolution().getCalendarField() >= DateTimeResolution.MINUTE
                    .getCalendarField()) {
                mins = createListBox();
                for (int i = 0; i < 60; i++) {
            ...
            if (getResolution().getCalendarField() >= DateTimeResolution.SECOND
                    .getCalendarField()) {
                sec = createListBox();
                for (int i = 0; i < 60; i++) {
